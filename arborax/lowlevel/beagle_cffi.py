...
class BeagleLikelihoodCalculator:
    ...
    def set_pattern_weights(self, weights=None):
        if weights is None:
            ...
            if len(weights) != self.pattern_count:
                raise ValueError(f"Weights length {len(weights)} != pattern count {self.pattern_count}")
    ...
    def get_all_partials_and_scales(self, node_count):
        ...
        """
        all_partials = np.zeros((node_count, self.pattern_count, self.state_count), dtype=np.float64)
    ...
    def update_partials(self, operations, operation_count=None):
        ...
        if count is None:
             raise ValueError("operation_count required when passing pre-computed operations")

        code = lib.beagleUpdatePartials(self.inst, c_ops, count, self.cumulative_scale_index)
