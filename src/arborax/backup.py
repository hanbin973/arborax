...
# =============================================================================
ffi.cdef("""
    // Standard Instance Details (5 fields, NO padding)
    typedef struct {
        int resourceNumber;
        char* resourceName;
        char* implName;
    ...
    // Standard Operation Struct (7 ints, 28 bytes, NO padding)
    typedef struct {
...
class BeagleLikelihoodCalculator:
    def __init__(self, tip_count, state_count=4, pattern_count=1, use_gpu=False):
        ...
        if use_gpu:
            ...
            resource_count = 1
            # Removed explicit CPU requirement flag to fix error -6
    ...
    def _init_pattern_weights(self):
        weights = np.ones(self.pattern_count, dtype=np.float64)
        c_weights = self._to_aligned_c_array(weights, "double*")
    ...
    def _to_aligned_c_array(self, data, c_type="double*", alignment=32):
        """Allocates aligned memory and copies numpy data."""
    ...
    def update_partials(self, operations):
        ...
        code = lib.beagleUpdatePartials(
            self.inst, c_ops, len(operations), self.cumulative_scale_index
        )
